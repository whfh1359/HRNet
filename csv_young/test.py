...
df = pd.read_csv('/app/csv/predict_addL.csv')
...
right_letter = list(zip(df['letter_R_x'], df['letter_R_y']))
left_letter = list(zip(df['letter_L_x'], df['letter_L_y']))
file = list(zip(map(str, name), middle, right_medial, left_medial, right_f, left_f, right_letter, left_letter))
...
for i in file:
    ...
    if filename == "000000002401":
        ...
        p_img = cv2.line(img, i[6], i[6], (255,255,0), 40)
        print(i[6],i[6])
        p_img = cv2.line(img,(2300, 434),(2300, 434), b, 40) #
#        p_img = cv2.line(img, i[7], i[7], b, 40)
#        000000002401, 833, 1005, 387, 1311, 1083, 977, 1501, 1311, 1446, 1339, 2351, 434, 0.9567193, 0.98299855, 0.94367784, 0.9882857, 0.06450432, 0.18884146
#        000000002212, 693, 949, 303, 1311, 1195, 949, 1557, 1311, 1195, 949, -267, 434, 0.36204788, 0.5576936, 0.49946356, 0.59243035, 0.186382, 0.07128566
        ...
        cv2.imwrite('/app/test/' + filename + '.jpg', p_img)
